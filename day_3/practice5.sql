...
/* 영향 받은 행: 0  찾은 행: 4  경고: 0  지속 시간 1 쿼리: 0.016 sec. */
select city.Name, city.Population, cl.language from city
	inner join countrylanguage as cl
	on cl.CountryCode = city.CountryCode
	order by city.Population desc limit 10;
/* 영향 받은 행: 0  찾은 행: 10  경고: 0  지속 시간 1 쿼리: 0.000 sec. */
select distinct city.Name, city.Population, cl.language from city
	inner join countrylanguage as cl
	on cl.CountryCode = city.CountryCode
	order by city.Population desc limit 10;
/* 영향 받은 행: 0  찾은 행: 10  경고: 0  지속 시간 1 쿼리: 0.187 sec. */
...
select distinct city.Name, city.Population, cl.language from city
	...
	order by city.Population desc limit 10;
/* SQL 오류 (1054): Unknown column 'T' in 'where clause' */
/* 영향 받은 행: 0  찾은 행: 5  경고: 0  지속 시간 1 of 2 쿼리: 0.000 sec. */
select * from countrylanguage limit 5;
select distinct city.Name, city.Population, cl.language from city
	inner join countrylanguage as cl
	on cl.CountryCode = city.CountryCode
	where cl.IsOfficial ='T'
	order by city.Population desc limit 10;
/* 영향 받은 행: 0  찾은 행: 15  경고: 0  지속 시간 2 쿼리: 0.015 sec. */
select distinct city.Name, city.Population, cl.language from city
	inner join countrylanguage as cl
	on cl.CountryCode = city.CountryCode
	where cl.IsOfficial ='T'
